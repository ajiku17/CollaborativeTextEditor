...
func main() {
	...
	manager := network.NewDocumentManager().(*network.DocumentManager)
	manager1 := network.NewDocumentManager().(*network.DocumentManager)
	doc := synceddoc.New(manager, onMessageReceive., nil, nil).(*synceddoc.SyncedDocument)
	doc1 := synceddoc.New(manager1, onMessageReceive, nil, nil).(*synceddoc.SyncedDocument)
	doc.Connect()
	go doc.SetChangeListener(nil)

	doc.InsertAtIndex(doc.GetDocument().Length(), "H")
	doc.InsertAtIndex(doc.GetDocument().Length(), "e")
